...
struct CoinTopView: View {
    var body: some View {
        VStack(alignment:.leading , spacing: 0){
            Text("Get Coins").font(.size24Semibold).foregroundColor(Color.primaryNavy)
            Spacer().frame(height: Spacing.s4)
            Text("Unlock premium features and extra sessions.").font(.size14Medium).foregroundColor(Color.gray600)
